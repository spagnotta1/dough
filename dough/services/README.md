...
| `health.py` | `score`, `cash_flow_stability`, `debt_burden`, `improvements` | app only | Feature 4. Gathers inputs and hands them to `dashboard_intel.health_score` — deliberately **not** a second scorer. **Every** surface showing "financial health" calls `score(window=…)`; the dashboard used to assemble its own inputs and the two pages disagreed by twenty points. The methodology table is in the module docstring, including why investment consistency is not scored and which two dimensions ignore the window |
